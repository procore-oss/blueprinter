...
describe Blueprinter::Extensions::OpenTelemetry do
  ...
  it 'should create a blueprinter.render span' do
    ctx = prepare(blueprint, nil, nil, { foo: 'Foo' }, {})
    ...
    called = subject.around(ctx) { :true }
    expect(called).to eq :true
  end

  it 'should create a blueprinter.object span' do
    ctx = prepare(sub_blueprint, nil, nil, { foo_obj: { name: 'Bar' } }, {})
    expect_any_instance_of(OpenTelemetry::Internal::ProxyTracer).
    ...
    called = subject.around_object(ctx) { :true }
    expect(called).to eq :true
  end

  it 'should create a blueprinter.collection span' do
    ctx = prepare(sub_blueprint, nil, nil, { foos: [{ name: 'Bar' }] }, {})
    expect_any_instance_of(OpenTelemetry::Internal::ProxyTracer).
    ...
    called = subject.around_collection(ctx) { :true }
    expect(called).to eq :true
  end

  it 'should create a blueprinter.extension span with an extension' do
    ctx = prepare(sub_blueprint, nil, nil, { foos: [{ name: 'Bar' }] }, {})
    expect_any_instance_of(OpenTelemetry::Internal::ProxyTracer).
  ...
  it 'should fire during render' do
    log = []
