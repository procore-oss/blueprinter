...
describe Blueprinter::V2::Extensions::Core::Conditionals do
  ...
  context 'fields' do
    ...
    it 'are allowed by default' do
      ...
      value = catch(sig) { subject.around_object_value(ctx) { 'Foo' } }
    ...
    it 'checks options field_if (Proc)' do
      ...
      value = catch(sig) { subject.around_object_value(ctx) { 'Bar' } }
  ...
  context 'objects' do
    ...
    it 'checks options object_if (Proc)' do
      ...
      value = catch(sig) { subject.around_object_value(ctx) { { name: 'Bar' } } }
      expect(value).to eq skip_field
    end
