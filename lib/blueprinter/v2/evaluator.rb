    ...
    class Evaluator
      ...
      def expand_partials(partials = self.partials)
        nodes.each_with_object([]) do |node, acc|
          ...
          self.nodes = exclude(blueprint.nodes, partial_exclusions(node))
      ...
      # Things that should be excluded from the partial in the given `DSL::Nodes::Use`
      def partial_exclusions(node)
        Exclusions.new(
          field_names: Set.new(node.exclude),
          fields: !node.fields,
