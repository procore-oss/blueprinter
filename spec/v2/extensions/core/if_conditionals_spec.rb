...
describe Blueprinter::V2::Extensions::Core::Conditionals do
  ...
  context 'fields' do
    ...
    it 'are allowed by default' do
      ...
      value = catch(skip_field) { subject.around_object_value(ctx) { 'Foo' } }
    ...
    it 'checks options field_if (Proc)' do
      ...
      value = catch(skip_field) { subject.around_object_value(ctx) { 'Bar' } }
  ...
  context 'objects' do
    ...
    it 'are allowed by default' do
      ctx = prepare(blueprint, {}, Blueprinter::V2::Context::Field, object, field)
      value = catch(skip_field) { subject.around_object_value(ctx) { { name: 'Foo' } } }
    ...
    it 'checks options object_if (Proc)' do
      ...
      value = catch(skip_field) { subject.around_object_value(ctx) { { name: 'Bar' } } }
      expect(value).to eq skip_field
    end
