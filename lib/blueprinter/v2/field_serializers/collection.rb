      ...
      class Collection < Base
        def serialize(ctx, result)
          value = catch Serializer::SKIP do
            ...
            @cond.around_collection_value(ctx) do
              @defaults.around_collection_value(ctx) do
                # perf boost by skipping `reduce_around` when no extensions use it
                if @hook_around_collection_value
                  @hooks.reduce_around(:around_collection_value, ctx) do
                    @extractor.around_collection_value ctx
                  end
                else
                  @extractor.around_collection_value ctx
                end
