...
describe Blueprinter::V2::Extensions::Core::Postlude do
  ...
  it 'should pass through the result by default for objects' do
    ctx = context.new(blueprint.new, nil, { name: 'Foo' }, nil, {})
    result = subject.output_object(ctx)
    expect(result).to eq({ name: 'Foo' })
  end

  it 'should pass through the result by default for collections' do
    ctx = context.new(blueprint.new, nil, [{ name: 'Foo' }], nil, {})
    result = subject.output_collection(ctx)
    expect(result).to eq([{ name: 'Foo' }])
  end

  it 'should look for a root option in the blueprint for objects' do
  ...
  it 'should look for a root option in the blueprint for collections' do
    blueprint.options[:root] = :data
    ctx = context.new(blueprint.new, nil, [{ name: 'Foo' }], nil, {})
    result = subject.output_collection(ctx)
    expect(result).to eq({ data: [{ name: 'Foo' }] })
  ...
  it 'should look for a root option in the options over the blueprint' do
    blueprint.options[:root] = :data
  ...
  it 'should look for a meta option in the blueprint' do
    blueprint.options[:root] = :data
  ...
  it 'should look for a meta option in the options over the blueprint' do
    blueprint.options[:root] = :data
    blueprint.options[:meta] = { links: [] }
  ...
  it 'should look for a meta Proc option in the blueprint' do
    blueprint.options[:root] = :data
  ...
  it 'should look for a meta Proc option in the options' do
    blueprint.options[:root] = :data
