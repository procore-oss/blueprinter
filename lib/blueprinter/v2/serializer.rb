    ...
    class Serializer
      ...
      def object(object, options, instances, store)
        if @run_around_object
          ...
          hooks.around(:around_object, ctx) { serialize(object, options, instances, store) }
        else
      ...
      def collection(collection, options, instances, store)
        if @run_around_collection
          ...
          hooks.around(:around_collection, ctx) do
      ...
      def find_used_hooks!
        @run_around_object = hooks.has? :around_object
        @run_around_collection = hooks.has? :around_collection
