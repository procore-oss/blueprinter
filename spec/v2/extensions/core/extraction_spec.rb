...
describe Blueprinter::V2::Extensions::Core::Values do
  ...
  context 'fields' do
    ...
    it 'should extract a value using the default extractor' do
      ctx = prepare(blueprint, field, nil, object, {})
      expect(subject.field_value ctx).to eq 'Foo'
    end

    it 'should extract a value using a block' do
      field = blueprint.reflections[:default].fields[:foo2]
      ctx = prepare(blueprint, field, nil, object, {})
      expect(subject.field_value ctx).to eq 'value: Foo'
    end

    it 'should extract a value using the field options extractor' do
      blueprint.field :foo, extractor: my_extractor
      ctx = prepare(blueprint, field, nil, object, {})
      expect(subject.field_value ctx).to eq 'FOO'
    end

    it 'should extract a value using the blueprint options extractor' do
  ...
  context 'objects' do
    ...
    it 'should extract a value using a block' do
      ...
      ctx = prepare(blueprint, field, nil, object, {})
      expect(subject.field_value ctx).to eq 'name: Bar'
    end
  ...
  context 'collections' do
    ...
    it 'should extract a field using a block' do
      field = blueprint.reflections[:default].collections[:foos2]
      ctx = prepare(blueprint, field, nil, object, {})
      expect(subject.field_value ctx).to eq 'nums: 42'
    end
