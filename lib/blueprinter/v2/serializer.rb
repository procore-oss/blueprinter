      ...
      def call_blueprint(object, options, instances, store)
        ...
        result = ctx.store.fetch(blueprint.object_id).fetch(:fields).each_with_object({}) do |field, acc|
          ...
          when ObjectField
            ...
            ctx.value = field.blueprint.serializer.call(ctx.value, options, instances, store) if ctx.value
            acc[field.name] = ctx.value
          when Collection
            ...
            ctx.value = ctx.value.map { |val| field.blueprint.serializer.call(val, options, instances, store) } if ctx.value
            acc[field.name] = ctx.value
          end
        end
