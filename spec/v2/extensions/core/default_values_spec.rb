...
describe Blueprinter::V2::Extensions::Core::Values do
  ...
  context 'fields' do
    ...
    it 'should check with field options default_if (Proc) (default = options field_default)' do
      blueprint.field :foo, default_if: ->(ctx) { is? ctx, 'Foo' }
      ctx = prepare(blueprint, field, 'Foo', object, { field_default: 'Bar' })
      expect(subject.field_value ctx).to eq 'Bar'
    end

    it 'should check with field options default_if (Symbol) (default = options field_default)' do
      blueprint.field :foo, default_if: :foo?
      ctx = prepare(blueprint, field, 'Foo', object, { field_default: 'Bar' })
      expect(subject.field_value ctx).to eq 'Bar'
    end

    it 'should check with field options default_if (Proc) (default = field options default)' do
      blueprint.field :foo, default: 'Bar', default_if: ->(ctx) { is? ctx, 'Foo' }
      ctx = prepare(blueprint, field, 'Foo', object, {})
      expect(subject.field_value ctx).to eq 'Bar'
    end

    it 'should check with field options default_if (Symbol) (default = field options default)' do
      blueprint.field :foo, default: 'Bar', default_if: :foo?
      ctx = prepare(blueprint, field, 'Foo', object, {})
      expect(subject.field_value ctx).to eq 'Bar'
    end

    it 'should check with field options default_if (Proc) (default = blueprint options field_default)' do
      blueprint.field :foo, default: 'Bar', default_if: ->(ctx) { is? ctx, 'Foo' }
      blueprint.options[:field_default] = 'Bar'
      ctx = prepare(blueprint, field, 'Foo', object, {})
      expect(subject.field_value ctx).to eq 'Bar'
    ...
    it 'should check with field options default_if (Symbol) (default = blueprint options field_default)' do
      blueprint.field :foo, default: 'Bar', default_if: :foo?
      blueprint.options[:field_default] = 'Bar'
      ctx = prepare(blueprint, field, 'Foo', object, {})
      expect(subject.field_value ctx).to eq 'Bar'
    ...
    it 'should check with blueprint options field_default_if (Proc) (default = options field_default)' do
      blueprint.options[:field_default_if] = ->(ctx) { is? ctx, 'Foo' }
      ctx = prepare(blueprint, field, 'Foo', object, { field_default: 'Bar' })
      expect(subject.field_value ctx).to eq 'Bar'
    end

    it 'should check with blueprint options field_default_if (Symbol) (default = options field_default)' do
      blueprint.options[:field_default_if] = :foo?
      ctx = prepare(blueprint, field, 'Foo', object, { field_default: 'Bar' })
      expect(subject.field_value ctx).to eq 'Bar'
    end

    it 'should check with blueprint options field_default_if (Proc) (default = field options default)' do
      blueprint.options[:field_default_if] = ->(ctx) { is? ctx, 'Foo' }
      blueprint.field :foo, default: 'Bar'
      ctx = prepare(blueprint, field, nil, object, {})
      expect(subject.field_value ctx).to eq 'Bar'
    ...
    it 'should check with blueprint options field_default_if (Symbol) (default = field options default)' do
      blueprint.options[:field_default_if] = :foo?
      blueprint.field :foo, default: 'Bar'
      ctx = prepare(blueprint, field, nil, object, {})
      expect(subject.field_value ctx).to eq 'Bar'
    ...
    it 'should check with blueprint options field_default_if (Proc) (default = blueprint options field_default)' do
      blueprint.options[:field_default_if] = ->(ctx) { is? ctx, 'Foo' }
      blueprint.options[:field_default] = 'Bar'
      ctx = prepare(blueprint, field, nil, object, {})
      expect(subject.field_value ctx).to eq 'Bar'
    ...
    it 'should check with blueprint options field_default_if (Symbol) (default = blueprint options field_default)' do
      blueprint.options[:field_default_if] = :foo?
      blueprint.options[:field_default] = 'Bar'
      ctx = prepare(blueprint, field, nil, object, {})
      expect(subject.field_value ctx).to eq 'Bar'
  ...
  context 'objects' do
    ...
    it 'should check with field options default_if (Proc) (default = options object_default)' do
      blueprint.object :foo_obj, sub_blueprint, default_if: ->(ctx) { is? ctx, 'Foo' }
      ctx = prepare(blueprint, field, nil, object, { object_default: 'Bar' })
      expect(subject.object_value ctx).to eq 'Bar'
    end

    it 'should check with field options default_if (Symbol) (default = options object_default)' do
    ...
    it 'should check with field options default_if (Proc) (default = blueprint options object_default)' do
      blueprint.object :foo_obj, sub_blueprint, default_if: ->(ctx) { is? ctx, 'Foo' }
      blueprint.options[:object_default] = 'Bar'
      ctx = prepare(blueprint, field, nil, object, {})
      expect(subject.object_value ctx).to eq 'Bar'
    ...
    it 'should check with field options default_if (Symbol) (default = blueprint options object_default)' do
      blueprint.object :foo_obj, sub_blueprint, default_if: :foo?
      blueprint.options[:object_default] = 'Bar'
      ctx = prepare(blueprint, field, nil, object, {})
      expect(subject.object_value ctx).to eq 'Bar'
    ...
    it 'should check with blueprint options object_default_if (Proc) (default = options object_default)' do
      blueprint.options[:object_default_if] = ->(ctx) { is? ctx, 'Foo' }
      ctx = prepare(blueprint, field, nil, object, { object_default: 'Bar' })
      expect(subject.object_value ctx).to eq 'Bar'
    end

    it 'should check with blueprint options object_default_if (Symbol) (default = options object_default)' do
      blueprint.options[:object_default_if] = :foo?
      ctx = prepare(blueprint, field, nil, object, { object_default: 'Bar' })
      expect(subject.object_value ctx).to eq 'Bar'
    end

    it 'should check with blueprint options object_default_if (Proc) (default = field options default)' do
      blueprint.options[:object_default_if] = ->(ctx) { is? ctx, 'Foo' }
      blueprint.object :foo_obj, sub_blueprint, default: 'Bar'
      ctx = prepare(blueprint, field, nil, object, {})
      expect(subject.object_value ctx).to eq 'Bar'
    ...
    it 'should check with blueprint options object_default_if (Symbol) (default = field options default)' do
      blueprint.options[:object_default_if] = :foo?
      blueprint.object :foo_obj, sub_blueprint, default: 'Bar'
      ctx = prepare(blueprint, field, nil, object, {})
      expect(subject.object_value ctx).to eq 'Bar'
    ...
    it 'should check with blueprint options object_default_if (Proc) (default = blueprint options object_default)' do
      blueprint.options[:object_default_if] = ->(ctx) { is? ctx, 'Foo' }
      blueprint.options[:object_default] = 'Bar'
      ctx = prepare(blueprint, field, nil, object, {})
      expect(subject.object_value ctx).to eq 'Bar'
    ...
    it 'should check with blueprint options object_default_if (Symbol) (default = blueprint options object_default)' do
      blueprint.options[:object_default_if] = :foo?
      blueprint.options[:object_default] = 'Bar'
      ctx = prepare(blueprint, field, nil, object, {})
      expect(subject.object_value ctx).to eq 'Bar'
  ...
  context 'collections' do
    ...
    it 'should check with field options default_if (Proc) (default = options collection_default)' do
      blueprint.collection :foos, sub_blueprint, default_if: ->(ctx) { is? ctx, 'Foo' }
      ctx = prepare(blueprint, field, nil, object, { collection_default: 'Bar' })
      expect(subject.collection_value ctx).to eq 'Bar'
    end

    it 'should check with field options default_if (Symbol) (default = options collection_default)' do
    ...
    it 'should check with field options default_if (Proc) (default = blueprint options collection_default)' do
      blueprint.collection :foos, sub_blueprint, default_if: ->(ctx) { is? ctx, 'Foo' }
      blueprint.options[:collection_default] = 'Bar'
      ctx = prepare(blueprint, field, nil, object, {})
      expect(subject.collection_value ctx).to eq 'Bar'
    ...
    it 'should check with field options default_if (Symbol) (default = blueprint options collection_default)' do
      blueprint.collection :foos, sub_blueprint, default_if: :foo?
      blueprint.options[:collection_default] = 'Bar'
      ctx = prepare(blueprint, field, nil, object, {})
      expect(subject.collection_value ctx).to eq 'Bar'
    ...
    it 'should check with blueprint options collection_default_if (Proc) (default = options collection_default)' do
      blueprint.options[:collection_default_if] = ->(ctx) { is? ctx, 'Foo' }
      ctx = prepare(blueprint, field, nil, object, { collection_default: 'Bar' })
      expect(subject.collection_value ctx).to eq 'Bar'
    end

    it 'should check with blueprint options collection_default_if (Symbol) (default = options collection_default)' do
      blueprint.options[:collection_default_if] = :foo?
      ctx = prepare(blueprint, field, nil, object, { collection_default: 'Bar' })
      expect(subject.collection_value ctx).to eq 'Bar'
    end

    it 'should check with blueprint options collection_default_if (Proc) (default = field options default)' do
      blueprint.options[:collection_default_if] = ->(ctx) { is? ctx, 'Foo' }
      blueprint.collection :foos, sub_blueprint, default: 'Bar'
      ctx = prepare(blueprint, field, nil, object, {})
      expect(subject.collection_value ctx).to eq 'Bar'
    ...
    it 'should check with blueprint options collection_default_if (Symbol) (default = field options default)' do
      blueprint.options[:collection_default_if] = :foo?
      blueprint.collection :foos, sub_blueprint, default: 'Bar'
      ctx = prepare(blueprint, field, nil, object, {})
      expect(subject.collection_value ctx).to eq 'Bar'
    ...
    it 'should check with blueprint options collection_default_if (Proc) (default = blueprint options collection_default)' do
      blueprint.options[:collection_default_if] = ->(ctx) { is? ctx, 'Foo' }
      blueprint.options[:collection_default] = 'Bar'
      ctx = prepare(blueprint, field, nil, object, {})
      expect(subject.collection_value ctx).to eq 'Bar'
    ...
    it 'should check with blueprint options collection_default_if (Symbol) (default = blueprint options collection_default)' do
      blueprint.options[:collection_default_if] = :foo?
      blueprint.options[:collection_default] = 'Bar'
      ctx = prepare(blueprint, field, nil, object, {})
      expect(subject.collection_value ctx).to eq 'Bar'
