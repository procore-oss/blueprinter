...
describe Blueprinter::V2::Serializer do
  ...
  it 'should call field_value hooks, then formatters, then exclude_field? hooks' do
    ext = Class.new(Blueprinter::Extension) do
      def field_value(ctx)
        case ctx.value
  ...
  it 'should call object_value hooks then exclude_object? hooks' do
    ext = Class.new(Blueprinter::Extension) do
  ...
  it 'should call collection_value hooks then exclude_collection? hooks' do
    ext = Class.new(Blueprinter::Extension) do
  ...
  it 'should run around_object around all other serializer hooks' do
    ext = Class.new(Blueprinter::Extension) do
      def initialize(log)
        @log = log
      end
      ...
      def around_object(ctx)
        @log << "around_object (#{ctx.object[:name]}): a"
        yield
        @log << "around_object (#{ctx.object[:name]}): b"
      end
  ...
  it 'should run around_collection around all other serializer hooks' do
    ext = Class.new(Blueprinter::Extension) do
      def initialize(log)
        @log = log
      end
      ...
      def around_collection(ctx)
        @log << "around_collection (#{ctx.object.map { |x| x[:name] }.join(',')}): a"
        yield
        @log << "around_collection (#{ctx.object.map { |x| x[:name] }.join(',')}): b"
      end
    ...
    expect(log).to eq [
      'around_collection (Foo,Bar): a',
      'prepare (sans object)',
      'blueprint_input',
      'blueprint_output',
      'blueprint_input',
      'blueprint_output',
      'around_collection (Foo,Bar): b',
    ]
  end
