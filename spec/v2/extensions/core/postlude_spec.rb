...
describe Blueprinter::V2::Extensions::Core::Postlude do
  ...
  let(:context) { Blueprinter::V2::Context::Object }
  let(:object) { { name: 'Foo' } }
  ...
  it 'passes through the result by default for objects' do
    ctx = context.new(blueprint.new, fields, {}, object, 1)
    result = subject.around_serialize_object(ctx) { |object| object }
    expect(result).to eq({ name: 'Foo' })
  end

  it 'passes through the result by default for collections' do
    ctx = context.new(blueprint.new, fields, {}, [object], 1)
    result = subject.around_serialize_collection(ctx) { |collection| collection }
    expect(result).to eq([{ name: 'Foo' }])
  end
  ...
  it 'looks for a root option in the options over the blueprint' do
    blueprint.options[:root] = :data
    ctx = context.new(blueprint.new, fields, { root: :root }, object, 1)
    result = subject.around_serialize_object(ctx) { |object| object }
  ...
  it 'looks for a meta Proc option in the options' do
    blueprint.options[:root] = :data
    ctx = context.new(blueprint.new, fields, { root: :root, meta: ->(ctx) { meta_links } }, object, 1)
    result = subject.around_serialize_object(ctx) { |object| object }
