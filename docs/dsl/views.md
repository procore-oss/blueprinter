...
Blueprints can define views to provide different representations of the data. A view inherits everything from its parent but is free to override as needed. In addition to [fields](./fields.md), views can define [options](./options.md), [partials](./partials.md), [formatters](./formatters.md), [extensions](./extensions.md), and [nested views](#nested-views).

At the top level of every Blueprint is an implicit view called `default`. This view is used when no other is specified. All other views in the Blueprint inherit from it.
...
Views can exclude fields from parents, views they've included, or from [partials](./partials.md).
