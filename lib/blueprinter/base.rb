...
module Blueprinter
  class Base
    ...
    #   `if: :include_first_name?, or if: Proc.new { |field_name, user, options| options[:current_user] == user }).
    ...
    #   `unless: :include_first_name?, or unless: Proc.new { |field_name, user, options| options[:current_user] != user }).
    #   The method, proc or string should return or evaluate to a true or false value.
    # @yield [object, options] The object and the options passed to render are
    #   also yielded to the block.
    #
