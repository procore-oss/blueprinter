...
Extractors are [extensions](./extensions.md#extract_value) that pull field values from the objects you're serializing. The default extractor is smart enough for most use cases, but you can create custom extractors if needed. (Note that passing a block to a field completely [bypasses extractors](../dsl/fields.md#extracting-field-values).)
