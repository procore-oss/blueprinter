    ...
    klass.class_eval do
      ...
      let(:blueprint) do
        ...
        Class.new(Blueprinter::Blueprint) do
          ...
          object(:foo_obj2, test.sub_blueprint) { |ctx| "name: #{ctx.object[:foo_obj][:name]}" }
          collection(:foos2, test.sub_blueprint) { |ctx| "nums: #{ctx.object[:foos].map { |x| x[:num] }.map(&:to_s).join(',')}" }
